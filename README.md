...
- **2026-06-25**：新增网球球速测速，按击球分段计算每次击球峰值速度（km/h），输出 `ball_speed.json` 并在视频上叠加测速面板。
...
- **球速测速** - 在干净球轨迹基础上，按弹跳点和回合间隙把轨迹切成每次击球，用球场平面坐标（米）估算每次击球的峰值球速（km/h），并以测速面板形式叠加在视频上，结果写入 `ball_speed.json`。单机位只能得到二维平面投影速度，会低估真实三维球速，平击球比高弧线球更准确。
...
--ball-speed true|false         是否测量并显示球速（km/h），默认 true
...
- `ball_speed.json`：球速测速结果，包含每次击球的峰值/平均速度（km/h）、击球帧、球场坐标，以及整场最大/平均击球速度。
